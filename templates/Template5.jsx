// Template 5: Elegant Sidebar (left sidebar for profile, right content)
import { cn } from "@/lib/utils"

export default function Template5({ resume, className }) {
  const { name, title, summary, experience = [], education = [], skills = [] } = resume || {}
  return (
    <div className={cn("text-gray-900", className)}>
      <div className="grid grid-cols-3 gap-8">
        {/* Sidebar */}
        <aside className="col-span-1 rounded-md bg-muted p-5">
          <h1 className="text-2xl font-semibold">{name}</h1>
          <p className="text-sm text-muted-foreground">{title}</p>
          {summary && <p className="mt-3 text-sm leading-relaxed">{summary}</p>}

          <h2 className="mt-5 text-xs font-semibold tracking-wide text-gray-700">Skills</h2>
